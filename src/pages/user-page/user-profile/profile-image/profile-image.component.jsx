...
class ProfileImage extends React.Component {
   ...
   render() {
      const { userProfile: { photo_url, first_name, family_name } } = this.props;
      const { imageName, imageFormOpen, imageCropOpen, crop, src } = this.state
      return (
         <div className="profile--image" >
            {imageFormOpen
               ? <form className="photo--form" autoComplete="off" >
                  <input type="file" hidden onChange={this.handleImageChange} ref={el => this.imageInput = el} />
                  <div id="pickImage--warning">Veuillez selectionner une image avec un poids inférieur à 3 méga octets pour faciliter son traitement et son téléversement.</div>
                  <div id="pickImage">
                     <CustomSmallButton type="button" className="custom-small-button--negative--gray-blue" onClick={() => this.imageInput.click()} >Choisir une image</CustomSmallButton>
                  </div>
               ...
               : <>
                  {photo_url
                     ? <div className="photo--link">
                        <img className="link" src={`${process.env.REACT_APP_SERVER_ORIGIN}/${photo_url}`} alt={`${first_name} ${family_name}`} />
...
const mapDispatchToProps = dispatch => ({
   updateUserProfileImageAsync: userData => dispatch(updateUserProfileImageAsync(userData))
});
