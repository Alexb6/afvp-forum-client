...
class EmailVerification extends React.Component {
  ...
  handleVerifyEmail = async () => {
    ...
    try {
      ...
      if (this.props.emailVerifiedError)
        throw new Error(this.props.emailVerifiedError);
      this.setState({ isLoading: false });
      scrollToTop();
    } catch (err) {
      scrollToTop();
      if (
        this.props.emailVerifiedError.startsWith(
          "Votre courriel est déjà verifié"
        )
      ) {
        this.setState({
          isLoading: false,
          warning: this.props.emailVerifiedError,
        });
      } else {
        this.setState({
          isLoading: false,
          error: this.props.emailVerifiedError,
        });
      }
    }
  };
    ...
    return (
      <div className="email--verification container mt-5 mb-5">
        <ModalPopUp
          ...
          headerClass="valid"
          title="Validation du courriel"
          footerClose
        >
          Votre courriel est validé. Vous pouvez désormais vous connecter à
        ...
        <div className="email--verification-frame">
          ...
          <p>
            En cliquant le bouton ci-dessous, vous confirmerez votre courriel.
          </p>
          <CustomButton
            className="custom-button--positive--duck"
